...
//            "aaSorting": [[4, "desc"]],
                ...
                ajax: function (data, callback, settings) {
                    ...
                    obj.loginId = localStorage.loginId;
                    obj.token = localStorage.token;
                    obj.input = that.inputText;     //输入框
                    obj.searchSign = $('#selectVal').val(); //窜货类型
                    obj.area = $('#selectVal2').val();     //市场类型
                    //ajax请求数据
                    $.ajax({
                        type: "post",
                        url: global_path + "/cms/transregionalAnalysis/listTransregionalAnalysisByAgent",
                        cache: false,  //禁用缓存
                        //传入组装的参数
                        data:obj,
                        dataType: "json",
                        success: function (res) {
                            if(res.code === 200){
                                /* header数据*/
                                var topNum = res.data.countMap;
                                for(var i in topNum){
                                    if(topNum[i] === null || topNum[i] === ''){
                                        topNum[i] = '0';
                                    }
                                }
                                that.fleeGoodsCount = topNum.fleeGoodsCount;            //我窜其他
                                that.beFleeGoodsCount = topNum.beFleeGoodsCount;            //其他窜我
                                that.onHandCount = topNum.onHandCount;            //待处理
                                that.makeUpCount = topNum.makeUpCount;            //累计和解
                                that.compensateCount = topNum.compensateCount;            //累计赔偿
                                that.receiveCompensation = topNum.receiveCompensation;            //累计收到赔偿
                                // 动态添加市场类型城市
                                // $('#selectVal2').empty();
                                ...
                                for(var i=0;i<that.areas.length;i++){
                                    var areas = that.areas[i];
                                    addCity(areas,areas);
                                    // node = '<option value="'+areas+'">'+areas+'</option>';
                                    // $("#selectVal2>option").eq(0).after(node);
                                    // debugger;
                                    // 当市场类型不为空时追加
                                    // if($('#selectVal2').val() === ''){
                                ...
                                // var optionArr = $('#selectVal2 option');
                                // console.log(optionArr.length);
                                // for(var i=0;i<optionArr.length;i++){
                                //     if(optionArr[i].text === areas || optionArr[i].value == areas){
                                //         return false;
                                //     }
                                // }
                                ...
                                that.max = Math.max.apply( Math, that.arrCount );  //找到被窜货市场数量中的最大值
                                // 窜货市场柱状图数据
                                ...
                                that.max2 = Math.max.apply( Math, that.arr2Count );  //找到窜货市场数量中的最大值
                                that.bar1();
                                that.bar2();
                                var returnData = {};
                ...
                columns: [
                    ...
                    //操作
                    {
                        ...
                        "render": function(data, type, full, meta) {
                            var node = '';
                            node = "<a href='../framePages/crossRegionAnalysis.html?deviceId="+full.deviceId+"' class='check'>查看</a>";
