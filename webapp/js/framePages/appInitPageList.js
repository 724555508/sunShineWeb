...
//            "aaSorting": [[4, "desc"]],
                ...
                ajax: function (data, callback, settings) {
                    ...
                    $.ajax({
                        type: "post",
                        url: global_path + "/cms/Advertisement/getInitPageList",
                        cache: false,  //禁用缓存
                        data: param,  //传入组装的参数
                        dataType: "json",
                ...
                columns: [
                    ...
                    //缩略图
                    {
                        ...
                        "render":function (data, type, full, meta) {
                            var node = '';
                            if(full.imgUrl != null){
                                node = '<img class="litimg" src='+full.imgUrl+'>';
                            }
                            return node;
                        }
                    },
                    //开始投放时间
                    {
                        ...
                        "render":function (data, type, full, meta) {
                            var node = ''
                            if(full.sign == 1){
                                return node = '<span style="color: red;font-weight: 800;">投放中</span>';
                            }
                            var nowTime = that.format("yyyy-MM-dd hh:mm:ss");
                            if(full.beginUpdateTime != null){
                                full.beginUpdateTime = full.beginUpdateTime.substring(0,full.beginUpdateTime.length-2);
                                node = full.beginUpdateTime;
                                if(full.beginUpdateTime < nowTime){
                                    return node = '已结束';
                                }
                            }
                            return node;
